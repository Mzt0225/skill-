...
```bash
git clone --depth 1 --filter=blob:none --sparse https://github.com/juliaError/econ-TopJournal-writing-Skill.git /tmp/econ-skills
cd /tmp/econ-skills
git sparse-checkout set skills/econ-writing-workflow
cp -r skills/econ-writing-workflow ~/.claude/skills/
rm -rf /tmp/econ-skills
```
...
git clone --depth 1 --filter=blob:none --sparse https://github.com/juliaError/econ-TopJournal-writing-Skill.git /tmp/econ-skills
cd /tmp/econ-skills
git sparse-checkout set skills/econ-write
cp -r skills/econ-write ~/.claude/skills/
rm -rf /tmp/econ-skills
```
...
git clone --depth 1 --filter=blob:none --sparse https://github.com/juliaError/econ-TopJournal-writing-Skill.git /tmp/econ-skills
cd /tmp/econ-skills
git sparse-checkout set skills/cn-top-econ-writing
cp -r skills/cn-top-econ-writing ~/.claude/skills/
rm -rf /tmp/econ-skills
```
...
git clone --depth 1 --filter=blob:none --sparse https://github.com/juliaError/econ-TopJournal-writing-Skill.git /tmp/econ-skills
cd /tmp/econ-skills
git sparse-checkout set skills/econ-table-figure-design
cp -r skills/econ-table-figure-design ~/.claude/skills/
rm -rf /tmp/econ-skills
```
...
```bash
git clone --depth 1 https://github.com/caodoudou99/literature-review-econ-skill.git /tmp/lit-skill
cp -r /tmp/lit-skill/literature-review-econ-skill ~/.claude/skills/
rm -rf /tmp/lit-skill
```
...
> ⚠️ 以下命令在 **Claude Code 对话框**中直接输入，不是在终端里执行。
...
> 📌 **一句话总结：**  Skill 可以让 AI 从"通用写作助手"变成"论文专家"——它知道顶级期刊的标准、经济学家的写作习惯、以及中英文各自的行文规范。装上它，你的 AI 才真正懂如何写一篇论文。
